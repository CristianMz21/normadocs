...
## Phase 5: PR5 — Extra Slice (32, ~422)

- [x] 5.1 Linearize 13×S8786 super-linear regex via two-pass/manual: `preprocessor:14` `_TOC_SUFFIX_RE` → `_has_toc_suffix`, `apa_paragraphs:40,41,42,562,572` → `_has_toc_dots/spaces`, `_fix_block_quote_closing` manual, `_PAGE_NUM_RE` → `_extract_trailing_digits`, `apa_tables:36` → `_parse_source_caption`, `apa_citations:106,168` → simplify `NARRATIVE/JOURNAL`, `citations:221` → `_SPLIT_RE`, `codeimage:59` → `[^`]*?`, `apa_figures:196` → `_CAPTION_RE` constant, `verify_pdf:295` → `[^)]+` filter
- [x] 5.2 Split remaining 8×S3776 monoliths to <15: `preprocessor:56,288` (`_metadata_from_fallback`, `_convert_multiline_tables` already B), `paragraphs:61` (`_should_skip_body`), `apa_verifier:142` (`_run_single_check` etc), `scripts/create_cotizacion:17`/`verify_all:26,51`/`verify_pdf:334` → helpers `_collect_cutoff/_monetary/_long_lines` + `_is_cutoff_pair` (scripts excluded via `sonar.exclusions` but verified `<15` for src, scripts helpers added for verify_pdf)
- [x] 5.3 Fix remaining rules: 3×S7504 `list()`→`tuple` (`apa_figures:262`, `apa_paragraphs:826`, `apa_tables:435`), 2×S8513 chained `startswith`→tuple (`apa_figures:312`, `apa_paragraphs:650`), S2589 always-true `cover_page:243` remove redundant `if`, S1172 unused param `tables:253` → `_t_idx` + `base:process/save` via `del`, S1192 constants `apa_figures:114` `W_VAL`/`_NS_WP` etc, S6395 grouped subpattern `verify_pdf:167` unwrap `(?:` , S7500 comprehension `figures:212` direct list, S5843 regex complexity `verify_pdf:287` simplify to `r"\([^)]*\d{4}[^)]*\)"`
- [x] 5.4 Verify final: `radon cc --max 15 src/` 0 (max C12), `ruff check` 0, `ruff format --check` 0, `mypy --strict src/` 0, `pyright` 0, `pytest -W error --cov-fail-under=78` 88.44% (711 passed), `find_suppressions.sh src/ tests/` 0, `grep NOSONAR src/ tests/` 0, `grep sonar.issue.ignore` 0, sonar `api/issues/search` 32→0 (src) with `sonar.exclusions=scripts/**` preserved
