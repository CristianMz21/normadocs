...
Write in Markdown. Generate reproducible academic documents and verify the selected rules.
...
| **NormaDocs** | ✅ Profile-specific formatting + APA verifier | ✅ BibTeX + CSL | ✅ Full pipeline |

**What you get:**
- Standard-specific margins, fonts, and spacing
- Strict APA structure and formatting verification for PDF output
...
**Output**: `ExportDocs/document_APA7ESTUDIANTE.docx` with APA-formatted cover page, double spacing, and proper margins.

For a complete report, include `Introducción`, `Desarrollo`, `Conclusiones` and
`Referencias`; see the [AI agent guide](docs/src/ai-agent.md).
...
```bash
# Python PDF backend + strict APA PDF verifier
pip install "normadocs[pdf,pdf-verifier]"

# Or install LibreOffice (recommended system backend)
...
Options:
  ...
  --apa-strict/--no-apa-strict      Strict APA 7 verification (default: enabled)
  --verify-apa/--no-verify-apa       Enable/disable APA verification for PDF/all
  --apa-report FILE                 Save the APA verification report as Markdown
  --language-tool LANG              Enable LanguageTool grammar checking
```
...
# Strict APA 7 validation is enabled by default for PDF/all output
normadocs paper.md --format all --apa-report apa-report.md
...
formatter = get_formatter("apa7estudiante", "output.docx")
...
Use YAML frontmatter as the primary metadata contract. Legacy positional metadata
is supported for older files, but frontmatter is safer for people and AI agents:

```markdown
---
title: "Document Title"
author: "Author Name"
affiliation: "Academic Program"
...
Author, A. (2026). Work title. Publisher.
```

Legacy positional metadata is still accepted for older files, but YAML
frontmatter is the recommended contract for automation.
...
Agent integration guide: [docs/src/ai-agent.md](docs/src/ai-agent.md)
